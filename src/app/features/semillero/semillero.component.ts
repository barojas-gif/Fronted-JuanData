...
import { ModalDocSemilleroComponent } from "./modal-doc-semillero/modal-doc-semillero.component";
  ...
  cargarDocumentosPorUsuario(idUsuario: number) {
    this.semilleroService.getDocumentosPorUsuario(idUsuario).subscribe({
      next: (docs) => {
        ...
        this.documentosPorCategoria = {};
        this.categorias.forEach(cat => {
          // DocumentoSemilleroDTO no trae 'categoria' (solo lo tiene DocumentosEntity/DocumentosDTO)
          this.documentosPorCategoria[cat] = (docs as any[]).filter(d => d.categoria === cat);
        });
      },
      error: (err) => {
