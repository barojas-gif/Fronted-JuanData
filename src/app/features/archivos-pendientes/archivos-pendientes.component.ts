...
import { DocumentoPendiente } from '../../core/models/documento.model';

@Component({
  selector: 'app-archivos-pendientes',
  standalone: true,
  imports: [CommonModule, MenuComponent, ModalMensajeComponent, FormsModule],
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
  templateUrl: './archivos-pendientes.component.html',
  styleUrls: ['./archivos-pendientes.component.css']
})
export class ArchivosPendientesComponent implements OnInit {
  ...
  // --- Variables del Modal ---
  mostrarModal = false;
  documentoSeleccionado: DocumentoPendiente | null = null;
  ...
  // --- Lógica del Modal ---
  abrirModal(documento: DocumentoPendiente, accion: 'aceptar' | 'rechazar') {
  ...
  get documentosFiltrados(): DocumentoPendiente[] {
    ...
    // Ordenamiento
    docs.sort((a, b) => {
      let valA: any = a[this.columnaOrden as keyof DocumentoPendiente];
      let valB: any = b[this.columnaOrden as keyof DocumentoPendiente];
  ...
  documentosPaginados(): DocumentoPendiente[] {
  ...
  abrirDocumento(doc: DocumentoPendiente) {
