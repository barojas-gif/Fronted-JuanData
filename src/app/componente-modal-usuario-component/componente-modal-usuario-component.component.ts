...
@Component({
  selector: 'app-componente-modal-usuario-component',
  standalone: true,
  imports: [FormsModule, CommonModule],
  templateUrl: './componente-modal-usuario-component.component.html',
  styleUrl: './componente-modal-usuario-component.component.css'
})
export class ComponenteModalUsuarioComponentComponent
  implements OnInit, OnChanges {
  semillerosCargados = false;


  @Input() mostrar = false;
  @Input() usuarioExistente?: UsuarioDTO;
  ...
  asignarDatosEdicion() {
    ...
    const u: any = this.usuarioExistente;
    ...
    const idSemillero =
      u.id_semillero ??
      u.idSemillero ??
      u.semillero?.id_semillero ??
      u.semillero?.idSemillero ??
      u.semillero?.id ??
      null;

    this.id_semillero = idSemillero !== null ? Number(idSemillero) : null;
    ...
    console.log(
      'IDS DISPONIBLES:',
      this.semillerosFiltrados.map(
        s => s.id_semillero || s.idSemillero || s.id
      )
    );
  ...
  // componente-modal-usuario.component.ts
  ...
  filtrarSemilleros() {
    ...
    this.semillerosFiltrados = this.semilleros.filter(s => {
      const carreraId =
        s.carrera?.id_carrera ||
        s.carrera?.idCarrera ||
        s.carrera?.id;

      return String(carreraId) === String(this.id_carrera);
    });
  }
